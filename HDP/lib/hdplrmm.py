...
class Gaussian:
    ...
    def predictive_logpdf_multiple_data_points(self, data_x, data_y):
        num_data = len(data_x)
        assert len(data_y) == num_data
        assert data_x.shape == (num_data, self.dim)
        assert data_y.shape == (num_data,)
        sum_logpdf = 0
        for data_i in range(num_data):
            sum_logpdf += self.predictive_logpdf(data_x[data_i], data_y[data_i])
        return sum_logpdf

    def get_UCB(self, x, alpha):
        # print("shape x {}".format(x.shape))
        # print("shape self.covar {}".format(self.covar.shape))
        var = self._sigma_square + np.matmul(np.matmul(x.transpose(), self.covar), x)
        mean = x.transpose().dot(self.mean)
...
class GibbsSampler(object):
    ...
    """
    def __init__(self, snapshot_interval=1, dimension = 50,compute_loglik=False):
    ...
    def print_statistics(self):
        ...
        for user in range(self._n_kd.shape[1]):
            for model in range(self._n_kd.shape[0]):
                print("user {} : model {}: number of data points {}".format(self.userID_list[user], model, self._n_kd[model,user]))
    ...
    def add_data_point(self, userID, x, y, table_topic_for_selected_arm):
        assert len(table_topic_for_selected_arm) == 2
        # print(type(table_topic_for_selected_arm[0]))
        # assert type(table_topic_for_selected_arm[0]) == int and type(table_topic_for_selected_arm[1]) == int
        if userID not in self.userID_list:
            self.userID_list.append(userID)
        if self._corpus_x == {}:
            # first time we add any data
            # need to initialize some variables
            self._corpus_x[userID] = np.reshape(x, [1, self.d])
            self._corpus_y[userID] = np.reshape(y, [1,])
            self._D = 1  # total number of documents
            self._K = 1  # total number of topics
            assert self._K == 1
            # initialize the word count matrix indexed by topic id and word id, i.e., n_{\cdot \cdot k}^v
            # self._n_kv = np.zeros((self._K, self._V))
            # initialize the word count matrix indexed by topic id and document id, i.e., n_{j \cdot k}
            self._n_kd = np.zeros((1, 1)).astype(int)  # how many theta in j equals phi_k, number of customers eating dish k in restaurant j
            # initialize the table count matrix indexed by topic id, i.e., m_{\cdot k}
            self._m_k = np.zeros(1).astype(int)  # number of tables with dish k
            ...
            self._k_dt = {}
            # initialize the word count vectors indexed by document id and table id, i.e., n_{j t \cdot}
            self._n_dt = {}  # number of customers on table t in restaurant j


            # initialize the table information vector indexed by document and records down which table a word belongs to
            self._t_dv[userID] = np.array([table_topic_for_selected_arm[0]], dtype=np.int).astype(int)
            assert(len(self._t_dv[userID]) == 1)
            assert (self._t_dv[userID] == np.array([0]))
            # self._k_dt records down which topic a table was assigned to
            assert table_topic_for_selected_arm[0] == 0
            self._k_dt[userID] = np.array([table_topic_for_selected_arm[1]]).astype(int)
            assert(len(self._k_dt[userID]) == 1)
            assert(self._k_dt[userID] == np.array([0]))

            # word_count_table records down the number of words sit on every table
            self._n_dt[userID] = np.array([len(self._corpus_x[userID])], dtype=np.int)
            assert(len(self._n_dt[userID]) == 1)
            assert(self._n_dt[userID] == np.array([1]))

            # in this way, user that comes in first will have small index
            # otherwise if we directly use userID as index, it will be messy, we will have to create large matrix
            # if the userID is not even an integer, then it won't work at all
            assert table_topic_for_selected_arm[1] == 0
            assert self.userID_list.index(userID) == 0
            self._n_kd[table_topic_for_selected_arm[1], self.userID_list.index(userID)] = len(self._corpus_x[userID])
            assert self._n_kd.shape == (1,1)
            # here, document index equals topic index
            self._m_k[table_topic_for_selected_arm[1]] = 1
            # initialize Gaussian mixtures, each document belongs to one cluster
            self.params[table_topic_for_selected_arm[1]] = Gaussian(X=np.zeros((0, len(x))), Y=np.zeros(0))

        else:
            # not first time to add data point
            if userID not in self._corpus_x:
                # first time to add data point for userID
                self._corpus_x[userID] = np.reshape(x, [1, self.d])
                self._corpus_y[userID] = np.reshape(y, [1, ])

                self._D += 1  # total number of documents
                assert self._D == len(self._corpus_x)

                # initialize the table information vector indexed by document and records down which table a word belongs to
                assert userID not in self._t_dv
                self._t_dv[userID] = np.array([table_topic_for_selected_arm[0]], dtype=np.int).astype(int)
                assert(len(self._t_dv[userID]) == 1)
                assert (self._t_dv[userID] == np.array([0]))
                # self._k_dt records down which topic a table was assigned to
                assert table_topic_for_selected_arm[0] == 0
                assert userID not in self._k_dt
                self._k_dt[userID] = np.array([table_topic_for_selected_arm[1]]).astype(int)
                assert(len(self._k_dt[userID]) == 1)
                # assert(self._k_dt[userID] == np.array([0]))

                # word_count_table records down the number of words sit on every table
                assert userID not in self._n_dt
                self._n_dt[userID] = np.array([len(self._corpus_x[userID])], dtype=np.int)
                assert(len(self._n_dt[userID]) == 1)
                assert(self._n_dt[userID] == np.array([1]))

                assert table_topic_for_selected_arm[1] <= len(self._m_k)
                if table_topic_for_selected_arm[1] < len(self._m_k):
                    self._m_k[table_topic_for_selected_arm[1]] += 1
                else:
                    self._m_k = np.append(self._m_k, [1], axis=0)
                    self._K += 1
                    self.params[table_topic_for_selected_arm[1]] = Gaussian(X=np.zeros((0, len(x))), Y=np.zeros(0))

                # since this is a new group, we need to expand self._n_kd
                self._n_kd = np.hstack((self._n_kd, np.zeros((self._n_kd.shape[0], 1)))).astype(int)
                assert table_topic_for_selected_arm[1] <= self._n_kd.shape[0]
                assert self.userID_list.index(userID)+1 == self._D
                if table_topic_for_selected_arm[1] < self._n_kd.shape[0]:
                    self._n_kd[table_topic_for_selected_arm[1], self.userID_list.index(userID)] = 1
                else:
                    self._n_kd = np.vstack((self._n_kd, np.zeros((1, self._D)))).astype(int)
                    self._n_kd[table_topic_for_selected_arm[1], self.userID_list.index(userID)] = 1
                assert self._n_kd.shape[0] == self._K
            else:
                # not first time to add data point for userID
                self._corpus_x[userID] = np.append(self._corpus_x[userID], np.reshape(x, [1, self.d]), axis = 0)
                self._corpus_y[userID] = np.append(self._corpus_y[userID], np.reshape(y, [1, ]), axis = 0)
                # self._D remain unchanged
                assert self._D == len(self._corpus_x)
                # we need to expand self._t_dv[userID] every time there is new data point
                self._t_dv[userID] = np.append(self._t_dv[userID], [table_topic_for_selected_arm[0]], axis=0)

                # if current word sits on a new table
                if table_topic_for_selected_arm[0] == len(self._k_dt[userID]):
                    new_table = True
                    # expand the vectors to fit in new table
                    self._n_dt[userID] = np.hstack((self._n_dt[userID], [1])).astype(int)
                    self._k_dt[userID] = np.hstack((self._k_dt[userID], [table_topic_for_selected_arm[1]])).astype(int)

                    assert (len(self._n_dt) == self._D and np.all(self._n_dt[userID] >= 0))
                    assert (len(self._k_dt) == self._D and np.all(self._k_dt[userID] >= 0))
                    assert (len(self._n_dt[userID]) == len(self._k_dt[userID]))
                else:
                    new_table = False
                    self._n_dt[userID][table_topic_for_selected_arm[0]] += 1
                    assert table_topic_for_selected_arm[1] == self._k_dt[userID][table_topic_for_selected_arm[0]]

                assert table_topic_for_selected_arm[1] <= len(self._m_k)
                if table_topic_for_selected_arm[1] < len(self._m_k):
                    new_topic = False
                    if new_table:
                        # if the data point sits at a new table
                        self._m_k[table_topic_for_selected_arm[1]] += 1
                else:
                    new_topic = True
                    self._m_k = np.append(self._m_k, [1], axis=0).astype(int)
                    self._K += 1
                    self.params[table_topic_for_selected_arm[1]] = Gaussian(X=np.zeros((0, len(x))), Y=np.zeros(0))
        ...
        self.params[table_topic_for_selected_arm[1]].add_point(x, y)

    """"
    select a linear regression model for userID based on his/her recent tau data points
    """
    def select_model(self, userID, tau):
        # the user might have data or may not have data
        if userID in self._corpus_x:
            # sample t using recent tau data
            user_tau_data_x = self._corpus_x[userID][-tau:] # gives us all the data if tau > len(self._corpus_x[userID])
            user_tau_data_y = self._corpus_y[userID][-tau:]
            ...
            for k in range(self._K):
                flog[k] = self.params[k].predictive_logpdf_multiple_data_points(user_tau_data_x, user_tau_data_y)
                try:
            ...
            base_distribution = Gaussian(X=np.zeros((0, user_tau_data_x.shape[1])), Y=np.zeros(0))
            f_new_topic = np.exp(base_distribution.predictive_logpdf_multiple_data_points(user_tau_data_x, user_tau_data_y))
            ...
            table_probability_log = np.zeros(len(self._k_dt[userID]) + 1, dtype=np.float64)
            for t in range(len(self._k_dt[userID])):
                if self._n_dt[userID][t] > 0:
                    # if there are some words sitting on this table,
                    # the probability will be proportional to the population
                    assigned_topic = int(self._k_dt[userID][t])
                    assert (assigned_topic >= 0 or assigned_topic < self._K)
                    # table_probability[t] = f[assigned_topic] * self._n_dt[document_index][t]
                    table_probability_log[t] = flog[assigned_topic] + np.log(self._n_dt[userID][t])
                else:
            ...
            table_probability_log[len(self._k_dt[userID])] = np.log(self._alpha) + np.log(f_new_table)
            ...
            if new_table == len(self._k_dt[userID]):
                ...
                if new_topic == self._K:
                    return new_table, int(new_topic), Gaussian(X=np.zeros((0, user_tau_data_x.shape[1])), Y=np.zeros(0))
                else:
                    return new_table, int(new_topic), self.params[new_topic]
            else:
                # an existing table
                return new_table, int(self._k_dt[userID][new_table]), self.params[self._k_dt[userID][new_table]]

        else:
            if (self._corpus_x == {}):
                # this is the first iteration, we don't have any data or model yet
                new_table = 0
                new_topic = 0
                model = Gaussian(X=np.zeros((0, self.d)), Y=np.zeros(0))
                return new_table, new_topic, model
            else:
                # new user
                # select the most popular dish across all users
                new_table = 0
                new_topic = np.argmax(self._m_k)
                return new_table, int(new_topic), self.params[new_topic]
                ...
                for table_index in np.random.permutation(range(len(self._k_dt[document_index]))):
                    ...
                    if self._n_dt[document_index][table_index] > 0:
                        ...
                        if new_topic != old_topic:
                            ...
                            # print("self.userID_list.index({}) = {}".format(document_index, self.userID_list.index(document_index)))
                            self._n_kd[old_topic, self.userID_list.index(document_index)] -= self._n_dt[document_index][table_index]
                            self._n_kd[new_topic, self.userID_list.index(document_index)] += self._n_dt[document_index][table_index]
    ...
    def update_params(self, document_index, word_index, update):
        ...
        self._n_kd[topic_id, self.userID_list.index(document_index)] += update
    ...
    # def update_corpus(self, data_x, data_y):
    #     if self._corpus_x != {}:
    #         assert len(self._corpus_x) == len(self._corpus_y)
    #         for group_id in data_x.keys():
    #             assert group_id in data_y
    #             if group_id in self._corpus_x:
    #                 assert group_id in self._corpus_y
    #                 self._corpus_x[group_id] = np.append(self._corpus_x[group_id], data_x[group_id], axis=0)
    #                 self._corpus_y[group_id] = np.append(self._corpus_y[group_id], data_y[group_id], axis=0)
    #             else:
    #                 assert group_id not in self._corpus_y
    #                 self._corpus_x[group_id] = data_x[group_id]
    #                 self._corpus_y[group_id] = data_y[group_id]
    #
    #     else:
    #         assert self._corpus_y == {}
    #         self._corpus_x = data_x
    #         self._corpus_y = data_y
    #
    #     self._D = len(self._corpus_x)
    #     assert self._D == len(self._corpus_y)